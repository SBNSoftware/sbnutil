...
# Parentage assignments (new pattern: reco1 is parent of downstream files)
# reco1: parent = initialfcl
echo -e " \"parents\": [\n\t{ \"file_name\": \"${PARENT_FILE_SAM}\" }\n  ]\n}" >> ${RECO1_MD}
# detsim: parent = initialfcl (not saved by default, kept unchanged)
echo -e " \"parents\": [\n\t{ \"file_name\": \"${PARENT_FILE_SAM}\" }\n  ]\n}" >> ${DETSIM_MD}
# downstream files: parent = reco1
echo -e " \"parents\": [\n\t{ \"file_name\": \"$(basename ${newReco1File})\" }\n  ]\n}" >> ${LARCV_MD}
echo -e " \"parents\": [\n\t{ \"file_name\": \"$(basename ${newReco1File})\" }\n  ]\n}" >> ${HIST_MD}
echo -e " \"parents\": [\n\t{ \"file_name\": \"$(basename ${newReco1File})\" }\n  ]\n}" >> ${CAF_MD}
echo -e " \"parents\": [\n\t{ \"file_name\": \"$(basename ${newReco1File})\" }\n  ]\n}" >> ${FLAT_MD}

echo "Parentage set: reco1 parent=${PARENT_FILE_SAM}"
echo "Parentage set: larcv/hist/caf/flat parent=$(basename ${newReco1File})"
