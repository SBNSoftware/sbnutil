...
# Configuration file for a SBND data decoding + reco1 + reco2
# Created on 2026-04-12 by Prabhjot Singh (prabhjot@fnal.gov)
# Based on metadata_postscripts_onestage.sh
...
# TODO: Remove below lines after successful test
#cafFile=''   # CAF trees
...
for mdFile in ${RECO1_MD} ${LARCV_MD} ${HIST_MD}; do
    echo -e "{" >> ${mdFile}
done
...
# Common tags
# TODO: Remove below lines after successful test
# mdFile in ${RECO1_MD} ${LARCV_MD} ${HIST_MD} ${CAF_MD} ${FLAT_MD} ; do
for mdFile in ${RECO1_MD} ${LARCV_MD} ${HIST_MD} ; do
...
for mdFile in ${RECO1_MD} ${LARCV_MD} ${HIST_MD} ; do
    ...
    echo -e " \"event_count\": ${MT_EVENTCOUNT}," >> ${mdFile}
    echo -e " \"runs\": [" >> ${mdFile}
