...
for mdFile in ${DETSIM_MD} ${RECO1_MD} ${LARCV_MD} ${HIST_MD} ${CAF_MD} ${FLAT_MD} ; do
    ...
    echo -e " \"mc.genie_tune\": \"${MT_GENIE_TUNE}\"," >> ${mdFile}
    echo -e " \"mc.genie_tag\": \"${MT_GENIE_TAG}\"," >> ${mdFile}
    echo -e " \"runs\": [" >> ${mdFile}
    echo -e "   [" >> ${mdFile}
...
if [[ ${COPY_BACK_DETSIM} == 1 ]] ; then
	...
	echo "DECLARING DETSIM ARTROOT FILE ${newDetsimFile}"
	samweb -e ${SAM_EXPERIMENT} declare-file ${DETSIM_MD}
	samweb -e ${SAM_EXPERIMENT} add-file-location $(basename ${newDetsimFile}) ${FULL_DETSIM_OUTDIR}
	echo -e "samweb -e ${SAM_EXPERIMENT} add_file_location $(basename ${newDetsimFile}) ${FULL_DETSIM_OUTDIR}"
	
	echo -e "Creating definition ${DETSIM_DTAG}..."
