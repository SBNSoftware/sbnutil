...
#======================================================================
#
# Name: icaruspoms_metadata.sh
#
# Purpose: Append SAM metadata overrides to a fcl file.
#
# Usage: icaruspoms_metadata_injector.sh [options]
#
# General options:
#
...
if [[ ${b_construct_metadata} == 1 ]] ; then
    samweb -e sbn get-metadata --json $(basename ${INPUTFILE})
else
    ...
    echo "MT_PRODUCTIONLABEL: ${MT_PRODUCTIONLABEL}"
    ...
    echo "services.FileCatalogMetadataSBN.Parameters: [[\"configuration.name\", \"$MT_CONFIGURATION\"], [\"data_stream\", \"$MT_DATASTREAM\"], [\"sbn_dm.event_count\", \"$MT_EVENTCOUNT\"], [\"sbn_dm.beam_type\", \"$MT_BEAMTYPE\"], [\"sbn_dm.detector\", \"$MT_DETECTOR\"]" | tee -a $INPUTFCLNAME
